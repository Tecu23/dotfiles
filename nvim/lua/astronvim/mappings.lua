...
maps.n["<leader>P"] = { "\"_dP", desc = "Copy but without replacing the buffer" }
maps.n["<C-d>"] = { "<C-d>zz", desc = "Keep cursor in the middle of the screen" }
maps.n["<C-u>"] = { "<C-u>zz", desc = "Keep cursor in the middle of the screen" }
...
if is_available "toggleterm.nvim" then
  ...
  if vim.fn.executable "node" == 1 then
    ...
    maps.t["<leader>tn"] = { function() utils.toggle_term_cmd "node" end, desc = "ToggleTerm node" }
  end
  local python = vim.fn.executable "python" == 1 and "python" or vim.fn.executable "python3" == 1 and "python3"
  if python then 
    maps.n["<leader>tp"] = { function() utils.toggle_term_cmd(python) end, desc = "ToggleTerm python" } 
    maps.t["<leader>tp"] = { function() utils.toggle_term_cmd(python) end, desc = "ToggleTerm python" } 
  end
  maps.n["<A-t>"] = { "<cmd>ToggleTerm direction=float<cr>", desc = "ToggleTerm float" }
  maps.t["<A-t>"] = { "<C-\\><C-n><cmd>ToggleTerm direction=float<cr>", desc = "ToggleTerm float" }
